...
export default function Footer() {
  ...
  const footerColumnsStagger = useScrollRevealStagger(4, { delay: 0 }, 'staggered');
  const bottomSectionReveal = useScrollReveal({ delay: 400 }, 'default');

  return (
    <footer className="relative border-t border-slate-200">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-10">
        <div 
          ref={footerColumnsStagger.containerRef}
        ...
        >
          ...
          <div className={footerColumnsStagger.getItemClasses(1)}>
          ...
          <div className={footerColumnsStagger.getItemClasses(2)}>
          ...
          <div className={footerColumnsStagger.getItemClasses(3)}>
            <p className="text-sm font-semibold text-slate-900">Legal</p>
            <ul className="mt-3 space-y-2 text-sm text-slate-700">
              <li><a className="hover:text-slate-900 transition-colors" href="/privacy-policy">Privacidade</a></li>
              <li><a className="hover:text-slate-900 transition-colors" href="/terms-of-service">Termos</a></li>
            </ul>
          </div>
        </div>
        ...
        <div 
          ref={bottomSectionReveal.ref}
          className={`mt-8 pt-6 border-t border-slate-200 ${bottomSectionReveal.animationClasses}`}
          style={bottomSectionReveal.animationStyles}
        >
          <div className="flex flex-col md:flex-row justify-between items-center gap-4">
            <p className="text-sm text-slate-600">
              © {new Date().getFullYear()} Esquads. Todos os direitos reservados.
            </p>
            <div className="flex items-center gap-4">
              <a 
                href="https://twitter.com/esquads" 
                target="_blank" 
              ...
              <a 
                href="https://linkedin.com/company/esquads" 
                target="_blank" 
                rel="noopener noreferrer"
                className="text-slate-600 hover:text-slate-900 transition-colors"
                aria-label="LinkedIn"
